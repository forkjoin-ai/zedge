...
- `server.ts` mounts the OpenAI-compatible, local MCP, edit-preview, TTS, Babelfish, workspace, mesh, and admin routes onto an `x-gnosis` host and can expose that host either directly or behind a native `gnosis-uring` public listener.
