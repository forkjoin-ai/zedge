...
- `companion-restart-policy.ts` centralizes the bounded restart thresholds so the manual supervisor and the MCP bridge make the same restart decisions.
...
- `aether-local-runtime.ts` centralizes the local SmolLM2 chat and MiniLM embedding fallback that `inference-bridge.ts` uses when mesh, edge, and Cloud Run paths miss.
- `inference-bridge.ts` owns the tier race, exposes the `wasm-local` model in the companion catalog, and now gives the local Aether runtime a real final fallback window before the echo belt-and-suspenders path.
