...
function writeFakeMoonshineProvider(path: string): void {
  writeFileSync(
    ...
    [
      ...
      '  process.exit(0);',
      '}',
      'if (command.includes("agent runs --json")) {',
      '  emit({ type: "run_record", workspace_path: process.cwd(), status: "completed", prompt: "write a pact smoke response", content: `${runId}: completed` });',
      '  emit({ type: "final", workspace_path: process.cwd(), content: "1 run ledger record(s)" });',
      '  process.exit(0);',
      '}',
