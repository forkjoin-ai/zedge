#!/bin/sh
# Ensure the companion sidecar answers on /health before starting the MCP stdio
...
if ! curl -sf "${HEALTH_URL}" >/dev/null 2>&1; then
  echo "[zedge] Companion not healthy; starting supervisor (log: ${LOG_FILE})" >&2
  ...
  while [ "$(date +%s)" -lt "${deadline}" ]; do
    if curl -sf "${HEALTH_URL}" >/dev/null 2>&1; then
      echo "[zedge] Companion is up" >&2
      break
    fi
    sleep 0.5
  done
  if ! curl -sf "${HEALTH_URL}" >/dev/null 2>&1; then
    echo "zedge: companion did not become healthy at ${HEALTH_URL} (see ${LOG_FILE})" >&2
    exit 1
  fi
fi
